...
covar2<-read.table("../Data/jordan_covariates_home.txt",header=TRUE)
...
jordan1 = read.table("jordan_data.txt",header=TRUE)
bigid = which(jordan1[,6]>28)
smallid = which(jordan1[, 6]<1)

jordan = jordan1[-c(smallid,bigid), ]
jordan[, 4] = jordan[, 4]/300 - 1
jordan[, 5] = jordan[, 5]/235 - 1
...
mu0.grid = matrix(c(jordan$Xdist,jordan$Ydist),ncol=2)
mu_pop = apply(GP.eigen.funcs(mu0.grid, n, a, b),2,sum)
mutc = matrix(0,nrow=L, ncol=num.type*p)
for(type in 1:2){
  for(k in 1:p) {
    jk=p*(type-1)+k
    mutc[,jk]= mu.tc(type, k, jordan, covar, n=n, a=a, b=b)
  }
}

#mu_type=sapply(1:num.type, function(i) mu.type.th(i,jordan,covar,L,n=n,a=a,b=b))
edxdy = (courtgrid[1,1]-courtgrid[2,1])^2
current.theta =esimat.ini(jordan, covar, courtmat, edxdy, num.type=2)
likelihood1 = rep(0,N)
likelihood2 = rep(0,N)
accept.pop = 0
...
for(sim in 1:N){
  ...
  mu.pop.new = current.theta[,1] + tau.pop/2*(matrix(c(-comp.mucoef.pop(current.theta,
                                                                        jordan,covar, intermat, dxdy) + mu_pop),ncol=1) - solve(var.pop)%*%current.theta[,1])
  temp.theta[,1] = mu.pop.new+rnorm(L, 0, sd=sqrt(tau.pop)) 
  mu.pop.old = temp.theta[,1] + tau.pop/2*(matrix(c(-comp.mucoef.pop(temp.theta,
                                                                     jordan,covar, intermat, dxdy) + mu_pop),ncol=1) - solve(var.pop)%*%temp.theta[,1])
  current.post.pop = comp.post.pop(current.theta,jordan,covar,intermat, dxdy) -  #current likelihood value
    mu_pop%*%current.theta[,1] + den.norm(current.theta[,1], var.pop)
  new.post.pop = comp.post.pop(temp.theta,jordan,covar,intermat, dxdy) -
  ...
  for(type in 1:2){
    for(k in 1:p) {
      ...
      varjk = sigmajk*jkmat
      temp.theta = current.theta
      new.mean = current.theta[,jk]+tauk[k]/2*(-comp.mucoef.tc(type,k,current.theta,jordan,covar, intermat, dxdy)+
                                                 mutc[,jk-1]-solve(varjk)%*%current.theta[,jk])
      temp.theta[,jk]= new.mean + rnorm(L,0,sd=sqrt(tauk[k])) 
      curr.mean = temp.theta[,jk]+tauk[k]/2*(-comp.mucoef.tc(type,k,temp.theta,jordan,covar,intermat, dxdy)+
                                               mutc[,jk-1] - solve(varjk)%*%temp.theta[,jk])
      new.post.tc = comp.post.type(type,temp.theta,jordan,covar,intermat, dxdy) -
        sum(mutc[,jk-1]*temp.theta[,jk]) + den.norm(temp.theta[,jk],varjk)
      curr.post.tc = comp.post.type(type,current.theta,jordan,covar,intermat, dxdy)-
  ...
  current.post=-comp.post.pop(current.theta, jordan,covar,intermat, dxdy)+  #current likelihood value
...
save(Theta.result,file="../Data/jordan_homelevel_theta.RData")
